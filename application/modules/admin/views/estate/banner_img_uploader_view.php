...
<div id="trainingrecords-attachments" target=".trainingrecords-attachment-list">
	...
	<form action="<?php echo site_url('admin/realestate/uploadbannerfile');?>" method="post" enctype="multipart/form-data">
	    <ol class="filelist">
	    </ol>	
...
jQuery(document).ready(function(){
	...
	jQuery('#'+tab+' > form').ajaxForm({
		...
		complete: function(xhr) {
			...
			if(response.error==0)
			{
				window.parent.jQuery('.banner-error<?php echo $count;?>').html('');
				window.parent.jQuery('.banner_photo<?php echo $count;?>').attr("value",response.name);
				window.parent.jQuery('.banner_photo<?php echo $count;?>').trigger('change');
			}
			else
			{
				...
				window.parent.jQuery('.banner-error<?php echo $count;?>').html(error);
			}
