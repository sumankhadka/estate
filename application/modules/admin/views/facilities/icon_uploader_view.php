...
<div id="trainingrecords-attachments" target=".trainingrecords-attachment-list">
	...
	<form action="<?php echo site_url('admin/realestate/uploadiconfile');?>" method="post" enctype="multipart/form-data">
	    <ol class="filelist">
	    </ol>	
	    	<input type="file" name="photoimg" style="height:auto;" >
	        <input type="submit" class="btn btn-info" value="Upload" style="margin-top:10px;">
        <p>Thumbnail must be of 32x32 pixels.</p>
	</form>    
	...
	jQuery('#'+tab+' > form').ajaxForm({
		...
		complete: function(xhr) {
			...
			if(response.error==0)
			{
				...
				window.parent.jQuery('#icon_input').attr("value",response.name);
				window.parent.jQuery('#icon_input').trigger('change');
			}
			else
			{
				if(response.error=='upload_invalid_min_dimensions')
					response.error = 'Image needs to be minimum 32x32 pixels.';
				var error = '<span style="color:red">'+response.error+'</span>';
				window.parent.jQuery('#icon-error').html(error);
			}
