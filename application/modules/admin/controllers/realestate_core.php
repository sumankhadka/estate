...
class Realestate_core extends CI_Controller {
...

	{
        ...
        // $value['extra_station'] = $this->realestate_model->get_extra_estate_by_id($id);
        $value['extra_stations'] = $this->realestate_model->get_trainline_stations($id);
        // var_dump($value['extra_stations']);
...

	{
		...
		$this->form_validation->set_rules('trainline', 'Trainline', 'required');
...

		{
			...
			else
			{
				...
				// $data['trainline'] 		= $this->input->post('trainline');

				// $meta_search_text		.= ' '.get_trainline_name_by_id($data['trainline']);
				...
				// $selectedStationIds 	= $this->input->post('selected_station');
	   //          $selectedTrainLineIds 	= $this->input->post('trainline');

	   //          $this->realestate_model->delete_staions_table($id);
	   //          $this->realestate_model->insertIntoExtraStationTable($id, $selectedStationIds,$selectedTrainLineIds);

				$selectedStationIds 	= array_unique($this->input->post('selected_station'));
	            $selectedTrainLineIds 	= array_unique($this->input->post('trainline'));
	            $this->realestate_model->delete_staions_table($id);
	            $this->realestate_model->insertIntoExtraStationTable($id, $selectedStationIds,$selectedTrainLineIds);
	            //trainline update end
	...
	public function addestate()
	{
...

		{
			...
			else
			{
                ...
                /*add trainline and station*/
				// $data['trainline'] 		= $this->input->post('trainline');

				// $meta_search_text	   .= ' '.get_trainline_name_by_id($data['trainline']);

				// /echo "<pre>";print_r($this->input->post());exit;
				//$station_id 			= $this->realestate_model->get_trainline_id_by_name($this->input->post('selected_station'),'station',$data['trainline']);
				//$data['station'] 			= $station_id;
				//$meta_search_text		.= ' '.$this->input->post('station');
				/*add trainline and station*/
				...
				//insert into extra station
				//in case of EDIT FORM, delete records first.
	            ...
	            $lastInsertedPostId = $this->realestate_model->insert_estate_meta($data);
	...
	#trainlines
	public function trainlines($start='0')

	{

        $data['title'] = 'All Trainlines';

        $value['posts'] = $this->realestate_model->get_all_trainlines_by_range($start,$this->per_page,'id');

        $total 				= $this->realestate_model->count_all_trainlines();

		$value['pages']		= configPagination('admin/realestate/trainlines',$total,5,$this->per_page);



        $data['content'] = $this->load->view('admin/estate/all_trainlines_view',$value,TRUE);

		$this->load->view('admin/template/template_view',$data);		

	}

	public function newtrainline($type='trainline')

	{
		$value['type'] = $type;

		$value['trainlines'] = $this->realestate_model->get_trainlines_by_type('trainline');

		$value['stations'] 	= $this->realestate_model->get_trainlines_by_type('stations');

		$this->load->view('admin/estate/new_trainline_view',$value);

	}

	public function savetrainline()
...

		{
			...
			else
			{
				...
				elseif($type=='station')
					...
					$parent = $this->input->post('trainline');
...

				{
					...
					$this->realestate_model->insert_trainline($data);
			...
			redirect(site_url('admin/realestate/newtrainline'));		

		}

	}

	public function edittrainline($type='trainline',$id='')

	{
		...
		$value['edittrainline'] = $this->realestate_model->get_trainline_by_id($id);

		$value['trainlines'] 	= $this->realestate_model->get_trainlines_by_type('trainline');

		$value['stations'] 		= $this->realestate_model->get_trainlines_by_type('station');

		$this->load->view('admin/estate/edit_trainline_view',$value);

	}

	public function updatetrainline()

	{
		...
		$this->form_validation->set_rules('trainline', 'Name', 'required');
...

		{
			...
			else
			{
				...
				$this->realestate_model->update_trainline($data,$id);
			...
			redirect(site_url('admin/realestate/edittrainline/'.$type.'/'.$id));		

		}

	}

	public function deletetrainline($page='0',$id='',$confirmation='')

	{
...

		{
			...
			$data['content'] = $this->load->view('admin/confirmation_view',array('id'=>$id,'url'=>site_url('admin/realestate/deletetrainline/'.$page)),TRUE);
...

		{
...

			{
				...
				{
					$this->realestate_model->delete_trainline_by_id($id);
			...
			redirect(site_url('admin/realestate/trainlines/'.$page));		
	...
	public function get_station_ajax($term='')

	{
		...
		$trainline = $this->input->post('trainline');

		$data = $this->realestate_model->get_trainlines_json($term,'station',$trainline);
