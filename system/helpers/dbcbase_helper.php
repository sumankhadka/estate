...
if ( ! function_exists('lang_key'))
{
    function lang_key($key='')
    {
        if(defined('LANG_ARRAY'))
        {
            ...
            return (isset($lang[$key]))?$lang[$key]:ucwords(str_replace('_',' ',$key));;
        }
        else
        {
            $CI = get_instance();
...
if ( ! function_exists('get_all_distances'))
{
	function get_all_distances()
	{
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('options',array('key'=>'distance_field','status'=>'1'));		
		return $query;
	}
}

if ( ! function_exists('get_distance_field_icon'))
{
	function get_distance_field_icon($id)
	{
		$CI = get_instance();
		$CI->load->database();
		$query = $CI->db->get_where('options',array('key'=>'distance_field','status'=>'1','id'=>$id));
		if($query->num_rows()>0) {
			$val = json_decode($query->row()->values);		
			return $val->{'icon'};
		}
		return '';
	}
